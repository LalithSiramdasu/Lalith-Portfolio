...
const configuredAdminUsername = import.meta.env.VITE_ADMIN_USERNAME?.trim();
const configuredAdminPassword = import.meta.env.VITE_ADMIN_PASSWORD;

// Preserve the current local fallback until admin auth is moved server-side.
const fallbackAdminUsername = 'lalithchinnu';
const fallbackAdminPassword = 'chinnu1810';

const expectedAdminUsername = normalizeUsername(configuredAdminUsername || fallbackAdminUsername);
const expectedAdminPassword = configuredAdminPassword || fallbackAdminPassword;
...
export const AdminLogin = ({ onLogin }: AdminLoginProps) => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        if (normalizeUsername(username) === expectedAdminUsername && password === expectedAdminPassword) {
